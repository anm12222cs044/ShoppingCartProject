function displayItems(filter_id)
{
	...
	if(myCookie!=""&&myCookie!=null)
	{
		...
		if(localStorage.getItem("finalHTML")!="")
		{
		...
		{
		document.getElementById(`i${prodArray[k]}`).value=qtyArray[k]	
		}
	}
		...
		for(let k in myObject)
		{
			if(myObject[k].filter_id===filter_id)
			{
					...
					for(let i in myBlocks)
					{
						...
						myBlocks[i].style.background.color="wheat"
					...
					for(let m in myImages)
					{
						...
						myImages[m].style.objectFit="contain"
						}
					}
...
function displayProduct(products)
{
	...
	return `<div id="p${products.product_id}">
	<H2>${products.product_name}</H2>
	<img src="${products.imagesrc}">
	<br>
	<HR style="margin-bottom:10px;">
	<span>₹${products.price}</span><button class="float-right btn btn-info" onclick="addToCart(${products.product_id},'${products.product_name}','${products.imagesrc}',${products.price})">Add-to-cart</button></div>`
...
function addToCart(product_id,product_name,imgsrc,price)
{
	...
	if(myUser!=""&myUser!=null)
	{
       ...
       if(productsArray.indexOf(String(product_id))!=-1)
		{
	...
	let localPrices=localStorage.getItem("prices").split(",")
	localPrices.push(price)
	localStorage.setItem("prices",localPrices)
	...
	returnDiv=`<div id="flexContainer">
			   ...
			   <hr><button class="badge badge-pill badge-danger" style="padding:10px; float:right;" onclick="removeItem(${product_id})"><i class="bi bi-trash3-fill"></i></button></div><div id="buttons" style="background-color:transparent;background-image:none;border:none;"><button class="btn btn-danger" onclick="clearCart()" style="float:left;margin:auto auto auto 10px;">Clear Cart</button>
			   ...
			   let myImagesSrcs=new Array()
			   myImagesSrcs.push(imgsrc)

			   localStorage.setItem("imgSrcs",myImagesSrcs)
...
function checkOut()
{
   ...
   {
	let cardDiv=`<img src="../Images/visa.jpg"><img src="../Images/mastercard.jpg"><img src="../Images/amex.jpg"><img src="../Images/discover.jpg">`
	...
	function paymentSuccessfull(total)
	{
		 alert("Thank You Shopping with us.Your order will be delivered Shortly!")
		 clearCart()
		 document.getElementById("finalBill").innerHTML=""
		 document.getElementById("finalBill").style.display="none"
	}
